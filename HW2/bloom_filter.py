...
class BloomFilter:
  ...
  def check_membership(self, elem):
    return self.array.get(cs5112_hash1(elem)%self.size) 
    & self.array.get(cs5112_hash2(elem)%self.size)
    & self.array.get(cs5112_hash3(elem)%self.size)
    #self.array.get(cs5112_hash1(elem))
